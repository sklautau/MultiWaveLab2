...
def get_modality_expanded_segments_columns(dataset_name, modality_prefixes=["ppg", "ecg", "bio"]):
    ...
    segments_columns = get_segments_columns(dataset_name)
...
def choose_feature_modalities(df: pd.DataFrame, modalities: list, always_keep=["GLC", "participant_id"]):
    ...
    """
    print("Modalities to keep:", modalities)
    cols_to_keep = [
...
def remove_metadata_columns_but_id_and_glc(df: pd.DataFrame,
                                           ...
                                           segments_columns: list[str]) -> pd.DataFrame:
...
def regression_evaluation_for_specific_modalities(train_csv: str,
                                                  modalities: list[str],
                                                  prefix_for_results_file: str,
                                                  output_file_name: str,
                                                  metadata_columns: list[str],
                                                  segments_columns: list[str]) -> None:
    """
    ...
    validation_csv = train_csv.replace("train", "validation")
    ...
    test_set = remove_metadata_columns_but_id_and_glc(
        test_set, metadata_columns, segments_columns)
    ...
    train_set = remove_metadata_columns_but_id_and_glc(
        train_set, metadata_columns, segments_columns)
    if is_there_validation:
        validation_set = remove_metadata_columns_but_id_and_glc(
            validation_set, metadata_columns, segments_columns)
    ...
    print(f"Saving nested CV results to: {output_file_name}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Read and process a features file."
    )
    parser.add_argument(
        "dataset_name",
    ...
    parser.add_argument(
        "train_csv",
        metavar="TRAIN_CSV",
        help="Path to the input train CSV file."
    )
    ...
    modalities = ["ppg", "ecg", "bioimp"]  # example modalities

    # Create a DatasetConfig instance
    metadata_columns = get_chosen_metadata_columns(args.dataset_name)

    modalities_for_segments_columns = modalities
    if "bioimp" in modalities:
        modalities_for_segments_columns.remove("bioimp")
        # replace "bioimp" with "bio" for the function call
        modalities_for_segments_columns.append("bio")

    segments_columns = get_modality_expanded_segments_columns(
        args.dataset_name, modalities)

    # print("metadata_columns", metadata_columns)
    # print("segments_columns", segments_columns)

    if args.dataset_name == "ieb2":
        # dictionary of all combinations of ECG and PPG modalities for ieb2 dataset
        modality_combinations = {
            "ecg": ["ecg"],
            "ppg": ["ppg"],
            "ecg_ppg": ["ecg", "ppg"],
        }
    elif args.dataset_name == "ieb3":
        # dictionary of all combinations of ECG, PPG, and Bioimp modalities for ieb3 dataset
        modality_combinations = {
            "ecg": ["ecg"],
            "ppg": ["ppg"],
            "bioimp": ["bioimp"],
            "ecg_ppg": ["ecg", "ppg"],
            "ecg_bioimp": ["ecg", "bioimp"],
            "ppg_bioimp": ["ppg", "bioimp"],
            "ecg_ppg_bioimp": ["ecg", "ppg", "bioimp"],
        }
    else:
        raise ValueError(f"Unknown dataset: {args.dataset_name}")

    prefix_for_results_file = "example_prefix"  # example prefix
    output_file_name = "nested_cv_results.csv"  # example output file name

    for modality_name, modalities in modality_combinations.items():
        print("###################################################")
        print(
            f"\n=== Running regression evaluation for modalities: {modalities} ===")
        print("###################################################")
        output_file_name = f"nested_cv_results_{modality_name}.csv"
        # execute regression evaluation
        regression_evaluation_for_specific_modalities(train_csv,
                                                      modalities,
                                                      prefix_for_results_file,
                                                      output_file_name,
                                                      metadata_columns,
                                                      segments_columns)
