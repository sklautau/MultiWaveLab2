...
class FpCollection:
    ...
    def get_fiducials(self, s: pyPPG.PPG):
        ...
        diastolicpeak = self.get_diastolic_peak(
            onsets, dicroticnotch, apg_fp.e)

        # Merge Fiducial Points
        keys = ('on', 'sp', 'dn', 'dp')
        dummy = np.empty(len(peaks))
        dummy.fill(np.nan)  # dummy.fill(np.NaN)
        n = 0
        for temp_val in (onsets, peaks, dicroticnotch, diastolicpeak):
            ppg_fp[keys[n]] = dummy
            ppg_fp.loc[ppg_fp.index[:len(temp_val)], keys[n]] = temp_val
            n = n+1
        ...
        for temp_sig in (ppg_fp, vpg_fp, apg_fp, jpg_fp):
            for key in list(temp_sig.keys()):
                ...
                fiducials.loc[fiducials.index[:len(temp_val)], key] = temp_val
    ...
    def get_peak_onset(self, peak_detector='PPGdet'):
        ...
        hr_win_v = []
        px = self.detect_maxima(
            x, 0, hr_win, peak_detector)  # detect all maxima
        if len(px) == 0:
            peaks = []
            onsets = []
            return peaks, onsets
        ...
        for win_no in range(0, len(win_starts) - 1):
            ...
            y1 = self.def_bandpass(
                curr_x, fs, 0.9 * up.fl_hz, 3 * up.fh_hz)   # Filter no.1
            # Estimate HR from weakly filtered signal
            hr = self.estimate_HR(y1, fs, up, hr_past)
            hr_past = hr
            all_hr[win_no] = hr
            ...
            y2 = self.def_bandpass(
                curr_x, fs, 0.9 * up.fl_hz, 2.5 * hr / 60)           # Filter no. 2
            # Estimate derivative from highly filtered signal
            y2_deriv = self.estimate_deriv(y2)
            ...
            p3 = self.detect_maxima(y3, 50, hr_win, peak_detector)
            ...
            all_p4 = np.concatenate(
                (all_p4, win_starts[win_no] + p4), axis=None)
        ...
        onsets, peaks = self.find_onsets(
            self.ppg, fso, up, peaks, 60/np.median(all_hr)*fs)
    ...
    def detect_maxima(self, sig: np.array, percentile: int, hr_win: int, peak_detector: str):
        ...
        if peak_detector == 'PPGdet':
            ...
            if hr_win == 0:
                ...
                max_pks = find_peaks(
                    sig+min(sig), prominence=tr2, distance=hr_win)[0]
    ...
    def elim_vlfs(self, s: np.array, up: DotMap, lower_cutoff: float):
        ...
        width = abs(up.paramSet.elim_vlf.Fpass -
                    up.paramSet.elim_vlf.Fstop)/(s.fs/2)
    ...
    def elim_vhfs(self, s: np.array, up: DotMap, upper_cutoff: float):
        ...
        width = abs(up.paramSet.elim_vhf.Fpass -
                    up.paramSet.elim_vhf.Fstop) / (s.fs / 2)
        [N, beta] = kaiserord(ripple, width)
        if N * 3 > len(s):
            N = round(N / 3)
        b = firwin(N, fc * 2 / s.fs, window=('kaiser', beta), scale=('True'))
    ...
    def estimate_HR(self, sig: np.array, fs: int, up: DotMap, hr_past: int):
        ...
        if (hr_past / 60 < up.fl_hz) | (hr_past / 60 > up.fh_hz):
            ...
            rel_els = np.where((fh >= hr_past / 60 * 0.5) &
                               (fh <= hr_past / 60 * 1.4))
    ...
    def find_onsets(self, sig: np.array, fs: int, up: DotMap, peaks: np.array, med_hr: float):
        ...
        while i < len(peaks):
            ...
            temp_oi1 = temp_oi0[np.where(
                before_peak * after_last_onset*SUT_time*DT_time)]
            if len(temp_oi1) > 0:
                if len(temp_oi1) == 1:
    ...
    def get_dicrotic_notch(self, peaks: np.array, onsets: list):
        ...
        # Low pass filtered signal with 20 cut off Frequency and 5 Hz Transition width
    ...
    def get_vpg_fiducials(self, onsets: list):
        ...
        nan_v[:] = np.nan  # nan_v[:] = np.NaN
        u, v, w = copy.deepcopy(nan_v), copy.deepcopy(
            nan_v), copy.deepcopy(nan_v),
        ...
        for i in range(0, len(onsets)-1):
            try:
                ...
                v_upper_bound = (
                    (onsets[i + 1] - onsets[i]) * upper_bound_coeff + onsets[i]).astype(int)
    ...
    def get_apg_fiducials(self, onsets: list, peaks: np.array):
        ...
        nan_v = np.empty(len(onsets)-1)
        nan_v[:] = np.nan  # nan_v[:] = np.NaN
        a, b, c, d, e, f = copy.deepcopy(nan_v), copy.deepcopy(nan_v), copy.deepcopy(
            nan_v), copy.deepcopy(nan_v), copy.deepcopy(nan_v), copy.deepcopy(nan_v)
        for i in range(0, len(onsets)-1):
    ...
    def get_jpg_fiducials(self, onsets: list, apg_fp: pd.DataFrame):
        ...
        for i in range(0, len(onsets) - 1):
            try:
                # p1 fiducial point
                ref_b = apg_fp.b[np.squeeze(np.where(np.logical_and(
                ...
                # p2 fiducial point
                ref_start = p1[i]
                ref_c = apg_fp.c[np.squeeze(np.where(np.logical_and(
                    apg_fp.c > onsets[i], apg_fp.c < onsets[i + 1])))]
                ref_d = apg_fp.d[np.squeeze(np.where(np.logical_and(
        ...
        for i in range(0, len(fiducials.on)):
            ...
            if correction.on[0]:
                try:
                    ...
                    min_loc = np.argmin(
                        self.ppg[fiducials.on[i]-win_onl:fiducials.on[i]+win_onr]) + fiducials.on[i]
                    if fiducials.on[i] != min_loc:
                        ...
                        fiducials.on[i] = np.argmax(self.jpg[int(
                            fiducials.a[i]) - win_a:int(fiducials.a[i])]) + int(fiducials.a[i]) - win_a
                except:
                    pass
            ...
            if correction.dn[0]:
                try:
                    temp_segment = self.ppg[int(
                        fiducials.sp[i]):int(fiducials.dp[i])]
                    ...
                    if len(min_dn) > 0 and diff_dn > round(self.fs / 100):
                        ...
                        try:
                            ...
                            fiducials.loc[i, 'dn'] = find_peaks(-self.ppg[strt_dn:stp_dn])[
                                0][-1] + strt_dn
                            if fiducials.loc[i, 'dn'] > min_dn:
                                fiducials.loc[i, 'dn'] = min_dn
                        except:
                            strt_dn = fiducials.e[i]
                            stp_dn = fiducials.f[i]
                            fiducials.loc[i, 'dn'] = np.argmin(
                                self.jpg[strt_dn:stp_dn]) + strt_dn
                            if fiducials.loc[i, 'dn'] > min_dn:
                                fiducials.loc[i, 'dn'] = min_dn
            ...
            if correction.w[0]:
                ...
                if fiducials.w[i] < fiducials.e[i]:
                    try:
                        fiducials.loc[i, 'w'] = [
                            np.argmax(self.vpg[int(fiducials.e[i]):int(fiducials.f[i])]) + fiducials.e[i]]
                    except:
                        pass
            ...
            if correction.v[0] and correction.w[0]:
                if fiducials.v[i] > fiducials.e[i]:
                    try:
                        fiducials.loc[i, 'v'] = [
                            np.argmin(self.vpg[int(fiducials.u[i]):int(fiducials.e[i])]) + fiducials.u[i]]
                        fiducials.loc[i, 'w'] = [find_peaks(
                            self.vpg[int(fiducials.v[i]):int(fiducials.f[i])])[0][0] + fiducials.v[i]]
                    except:
                        pass
            ...
            if correction.f[0]:
                try:
                    ...
                    temp_segment = self.apg[int(fiducials.e[i]):int(
                        fiducials.on[i]+temp_end)]
